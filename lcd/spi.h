#ifndef SPI_CONFIG_H
	#define SPI_CONFIG_H

	#include <avr/io.h>
	#include "config.h"

	/*
	 * Set to 1, spi api will work in master mode
	 * else in slave mode
	 */
	#define SPI_CONFIG_AS_MASTER 	1


	/*
	 * Config SPI pin diagram
	 */
	#define SPI_DDR			DDRB
	#define SPI_PORT		PORTB
	#define SPI_PIN			PINB
	#define SPI_MOSI		PB3
	#define SPI_MISO		PB4
	#define SPI_SCK			PB5

	// Beyond the default SPI portc C.
	#define SD_SPI_PORT		PORTC
	#define SD_SPI_DDR		DDRC

	#define LCD_SPI_PORT		PORTD
	#define LCD_SPI_DDR		DDRD

	// Chip select lines.
	#define DEFAULT_SPI_CS		PB2
	#define SD_SPI_CS		PC1
	#define LCD_SPI_CS		PD3
	#define RFID_SPI_CS		PB0
#endif

#ifndef SPI_H
	#define SPI_H

	#include <stdint.h>

	void spi_init();
	uint8_t spi_transmit(uint8_t data);

	#define default_cs_select() SPI_PORT &= ~(1 << DEFAULT_SPI_CS)
	#define default_cs_release() SPI_PORT |= (1 << DEFAULT_SPI_CS)

	#define sd_select() SD_SPI_PORT &= ~(1 << SD_SPI_CS)
	#define sd_release() SD_SPI_PORT |= (1 << SD_SPI_CS)

	#define lcd_release() LCD_SPI_PORT &= ~(1 << LCD_SPI_CS)
	#define lcd_select() LCD_SPI_PORT |= (1 << LCD_SPI_CS)

#endif
