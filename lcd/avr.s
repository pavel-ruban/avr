...
reset:					rjmp	asm_main		;1 0x000 RESET External Pin, Power-on Reset, Brown-out Reset and Watchdog System Reset
					...
					reti;	PCINT1_TOGGLE		;3 0x002 INT1 External Interrupt Request 1
					reti				;4 0x003 PCINT0 Pin Change Interrupt Request 0
					reti;rjmp	PCINT1_TOGGLE		;5 0x004 PCINT1 Pin Change Interrupt Request 1
...
sd_insert_str:
.asciz	"int: microsd is inserted\n\r"
sd_pull_out_str:
.asciz	"int: microsd is pulled out\n\r"
sd_str:
.asciz	"int: oops\n\r"
.align	2
...
PCINT1_TOGGLE:
					...
					sbis	SD_STATUS_PIN, SD_STATUS
					rjmp	SD_INSERED

					ldi	r24,	lo8(sd_pull_out_str)
					ldi	r25,	hi8(sd_pull_out_str)
					rcall	uart_str_send_P
					...
					rjmp	SD_ROUTINE_END
SD_INSERED:
					ldi	r24,	lo8(sd_insert_str)
					ldi	r25,	hi8(sd_insert_str)
					rcall	uart_str_send_P
					ldi	r18,		1
					sts	SD_INIT,	r18
SD_ROUTINE_END:
...
timers_init:				setbi	TIMSK0,	TOIE0				; Включим прерывания переполнения таймера 0
					...
					outi	ICR1H,	0x07
					outi	ICR1L,	0xD0				; при  делителе 64 совпадение раз в 100 миллисекунд (10 раз в секунду)
...
interrupt_init:
					;outi	EIMSK,	(1 << INT1)
					;outi	EICRA,	(1 << ISC10) | (1 << ISC11)
					outi	PCICR,	(1 << PCIE1)
					outi	PCMSK1,	(1 << PCINT10)
					ret
...
; Инициализация локальной перифeрии ------------------------------------------------------------------------
					sbi	LCD_RESET_DDR,		LCD_RESET
					cbi	LCD_RESET_PORT,		LCD_RESET
