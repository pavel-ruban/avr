...
#define enc28j60_rx() spi_transmit(0xff)
#define enc28j60_tx(data) spi_transmit(data)
...
uint16_t enc28j60_recv_packet(uint8_t *buf, uint16_t buflen)
{
	...
	if(enc28j60_rcr(EPKTCNT))
	{
		...
		//*term_var_info = *((uint8_t *) (&status));
		//if (!*term_var_info) *term_var_info = 0xbb;

		//*(term_var_info + 1) = *((uint8_t *) (&status + 1));
		//if (!*(term_var_info + 1)) *(term_var_info + 1)= 0xbb;
		//*(term_var_info + 2) = 0;
