...
//  @file	 	button.c
//  @author 		ATMEL Norway
//  @brief 		AVR Butterfly button handling routines
//
//  Target(s)...: ATmega169
//
...
//
//  Revisions...: 1.2
//  $Id: button.c,v 1.15 2007/06/04 15:12:18 brokentoaster Exp $
// 
//  YYYYMMDD - VER. - COMMENT                                       - SIGN.
//
//  20030116 - 1.0  - Created                                       - KS
//  20031009          port to avr-gcc/avr-libc                      - M.Thomas
//  20041105 - 1.2	  Altered for Butterfly MP3 use					- N.Lott	
// 
**/
//mtA
//#include <inavr.h>
...
extern volatile uint16	gPlayTimeSeconds;
extern volatile uint8 	gMode;		// mode of player, idle, playing, off, etc
// mt  unsigned char gButtonTimeout = FALSE;
volatile uint16 gButtonTimeout = FALSE;
volatile uint8 gButtonLongTimeout =FALSE; // this is to detect long presses on the button
//mtA
//char KEY = NULL;
//char KEY_VALID = FALSE;
volatile uint8 KEY = NULL;
volatile uint8 KEY_VALID = FALSE;
volatile uint8 KEY_SHIFT = FALSE;
//mtE
...
*/
    ...
    if(key != KEY_NULL)
    {
        ...
        {
			//gButtonLongTimeout = FALSE; // clear the hold down timer
    ...
    }else/*{ // make key only valid when released.
	    if(gButtonTimeout)  // gButtonTimeout is set in the LCD_SOF_interrupt in LCD_driver.c
        {	
            if (!KEY_VALID)
            {
