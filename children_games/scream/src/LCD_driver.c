...
// switch to filename rather than standard buffer
char gShowFilename;
...
//    static char auto_joystick_count;
	...
	c_flash=0; // mt
// NAL-  theese sections have been removed as they are now implemented in button.c
//
    ...
    if (gLCD_Update_Required == TRUE)
    {
        ...
        for (i = 0; i < 6; i++)
        {
            if ((gScroll+i) >= 0 && (!EOL))
            {
				...
				if (gShowFilename){
					c = FAT16_longfilename[i + gScroll];
				}else{
