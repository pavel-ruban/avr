...
TCNT:					.space	4,	0x00
SR_DATA_0:				.byte	0xFF			; Кольцевые буферы на прием и передачу для UART с использованием
SR_CNT:					.byte	0
...
RX_OK_LIGHT:				sbi	PORTB,		2
					reti

RX_OK_DARK:				cbi	PORTB,		2
					reti
...
io_ports_init:				sbi	SR_CLK_DDR,	SR_CLK		; Initialize ports settings & set their data
					sbi	SR_PL_DDR,	SR_PL
					cbi	SR_DATA_DDR,	SR_DATA
					sbi	SR_DATA_PORT,	SR_DATA

					sbi	DDRB,	1		; Initialize ports settings & set their data
					sbi	DDRB,	2
					sbi	DDRC,	1
...
; Инициализация локальной перифeрии ------------------------------------------------------------------------
	...
	;				rcall	uart_init
					...
					sbi	SR_CLK_PORT,	SR_CLK
					cbi	SR_PL_PORT,	SR_PL

					;cbi	LED_SELECT_PORT,	LED_SELECT
					;cbi	LED_PIN_PORT,	LED_PIN
					clr	r0
					clr	r1
	;				rcall sr_init
...
handle:
				 ...
				 ;	invb	LED_SELECT_PORT,	LED_SELECT
				; 	sbi	LED_SELECT_PORT,	LED_SELECT
				 ;	rcall	led_test
					rcall	sr_pl_test
